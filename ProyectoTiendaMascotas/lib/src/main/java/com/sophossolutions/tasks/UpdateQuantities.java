...
public class UpdateQuantities implements Task {
	...
	public <T extends Actor> void performAs(T actor) {
		...
		actor.attemptsTo(Click.on(BTN_HOME), Click.on(BTN_SHOPPINGCAR),
				Enter.theValue(String.valueOf(intQuantitiesPets)).into(TXT_QUANTITY), Click.on(BTN_UPDATE));
		actor.remember("strCostTotal",
				"$" + Double.parseDouble(Text.of(TXT_PRICE).viewedBy(actor).asString().replace("$", "0"))
						* intQuantitiesPets + "0");
	}
